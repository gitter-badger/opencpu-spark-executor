#R opencpu-spark-executor

The R opencpu-spark-executor (ROSE) is a Scala library offering the
full scientific computing power of the R programming language to
JVM-based Apache Spark batch and streaming applications.
...
### ROSE Library Spark Batch Usage
...
processing by calling the `analyze` operation provided by ROSE
on the RDD:
...
### ROSE Library Spark Streaming Usage
...
by calling the `analyze` operation provided by ROSE on each RDD within
the stream: 
...
Note again, the use here of the R function `fraud::score` is simply
...
### Spark Application Deployment Considerations
...
deployment configuration parallels the configuration required by Apache
...
your application will automatically be distributed for execution at random
